...
class AMD extends EventEmitter {
  ...
  loadPlugins(pluginPath = "./plugins") {
    try {
      ...
      console.log("❌ Plugin loader safe error:", e.message);
    }
  }
  ...
  async handleMessage(msg) {
    try {
      ...
      console.log("⚠️ AMD handler error (ignored):", err.message);
    }
  }

  /* =========================
     👀 STATUS HOOK v3 (REALISTIC)
  ========================= */
  statusHook() {
    try {
      this.sock.ev.on("messages.upsert", async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            if (type === "statusNotification" || msg.key?.remoteJid === "status@broadcast") {
      ...
      console.log("⚠️ Status hook disabled");
    }
  }
